...
//  코스 A(racing)=bestTime, 코스 B(hard)=bestTimeHard, 코스 C(serp)=bestTimeSerp 로 리더보드를 분리한다.
const RECORD_FIELD = { racing: "bestTime", hard: "bestTimeHard", serp: "bestTimeSerp" };
...
function sendStats(p) {
  ...
  const u = users[p.account.userId];
  if (!u) return;
  send(p, { type: "stats", proWins: u.proWins || 0, proPlays: u.proPlays || 0, bestMs: u.bestTime || 0, bestHardMs: u.bestTimeHard || 0, bestSerpMs: u.bestTimeSerp || 0, totalTime: liveTotalTime(p), lastLogin: u.lastLogin || 0 });
}
