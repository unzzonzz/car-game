...
/* --- 이동 예산(러버밴딩) : 속도핵 서버 차단 -----------------------------------
 *  클라 스크립트를 어떻게 고쳐도 서버가 아는 "원래 물리 한계"(최고속)를 넘는 이동은
 *  위치를 잘라내(clamp) 남들 화면·판정에 반영되지 않게 한다. 제재(킥)가 아니라 무력화라 오탐 피해 없음.
 *  - 크레딧(리키버킷) : 경과시간만큼 이동 예산이 쌓이고(px), 상한까지만 → 네트워크 지터로 패킷이
 *    몰려 도착해도 정상 주행은 절대 안 잘린다. 지속 속도만 최고속×여유로 캡.
 *  - 패킷 공백 > 1.2s(탭 전환/절전) → 기준점 재설정(예전 자동감지 무고밴 원인 해결법 그대로).
 *  - 서버가 아는 정당한 순간이동(입장/스폰/그리드 배치)은 sg 리셋으로 통과.
 *  - 클라 주도 텔레포트(R키 출발선 복귀)는 기록/충돌과 무관한 타임어택·테스트 모드에서만 인정(1초 쿨).
 *  ※ 차량 충돌(bump)을 다시 켜면 임펄스 순간속도가 캡을 넘을 수 있음 → 여유 상향 필요. */
const SPEED_MARGIN = 1.15;                                            // 지터/보간 오차 여유
const SPEED_CAP_PXS = MAX_LEGIT_PXS * SPEED_MARGIN;                   // 지속 속도 상한 ≈3105px/s
const SPEED_CREDIT_CAP = Math.round(SPEED_CAP_PXS * 0.35);            // 순간 몰아쓰기 상한(≈350ms 분)
const SPEED_GAP_REBASE_MS = 1200;                                     // 이 이상 공백 = 재설정
const TELEPORT_OK_MODES = new Set(["a1", "a2", "a3", "racing", "hard", "serp", "c1", "c2", "c3", "d1", "retro1", "retro2", "test"]);
// 수신 좌표에 이동 예산 적용 → 허용된(잘라낸) 좌표 반환. p.sg = { x, y, t, credit }
function speedGuard(p, x, y, now, wantsTeleport) {
  const sg = p.sg;
  const legitTeleport = wantsTeleport && TELEPORT_OK_MODES.has(p.mode) && now - (p.lastTpAt || 0) > 1000;
  if (!sg || now - sg.t > SPEED_GAP_REBASE_MS || legitTeleport) {
    if (legitTeleport) p.lastTpAt = now;
    p.sg = { x, y, t: now, credit: SPEED_CREDIT_CAP }; // 재설정 : 이 위치를 새 기준점으로
    return { x, y };
  }
  let credit = Math.min(sg.credit + ((now - sg.t) / 1000) * SPEED_CAP_PXS, SPEED_CREDIT_CAP);
  const dx = x - sg.x, dy = y - sg.y;
  const dist = Math.hypot(dx, dy);
  if (dist > credit) { // 예산 초과분 잘라냄 — 핵 유저만 여기 걸린다 (정상 최고속은 크레딧이 항상 앞섬)
    const k = dist > 0 ? credit / dist : 0;
    x = sg.x + dx * k; y = sg.y + dy * k;
    credit = 0;
  } else {
    credit -= dist;
  }
  p.sg = { x, y, t: now, credit };
  return { x, y };
}
...
function applyState(p, m) {
  if (!p.active) return;
  if (p.mode === "boss" && (p.bossSpec || p.bossDeadUntil)) return; // 사망/관전 중 위치 무시 (유령 주행 방지)
  ...
  // 이동 예산(러버밴딩) : 물리 최고속을 넘는 이동은 잘라냄 — 속도핵이 남들 화면/판정에 안 먹히게.
  //  입장 직후(state 없음)는 첫 패킷을 기준점으로 삼는다 (스폰/그리드 배치 커버).
  if (!p.state) p.sg = null;
  ({ x, y } = speedGuard(p, x, y, now, !!m.teleport));
...
function broadcastCounts() {
  const counts = { survival: 0, a1: 0, a2: 0, a3: 0, racing: 0, hard: 0, serp: 0, c1: 0, c2: 0, c3: 0, d1: 0, retro1: 0, retro2: 0, pro: 0, test: 0, rank: 0, boss: 0, plaza: 0, sumo: 0 };
  for (const [, p] of players) {
